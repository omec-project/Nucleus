...
Each state can be configured with a state guard timer duration value. The value is configured in the state machine data model. The framework starts and stops the timer through the entry and exit functions defined for the states. By default, the state guard timer feature will be ON. It can be turned off at the procedure level by adding a property 'stateGuardTimerFeature' at the procedure level and setting it to 'disabled'.
