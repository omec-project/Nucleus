...
*/
static int
ics_req_attach_processing(struct init_ctx_req_Q_msg *g_icsreq)
{
        log_msg(LOG_DEBUG,"Process Init Ctxt Req for Attach Request.");
        uint32_t length = 0;
        uint8_t *buffer = NULL;
	...
	struct s1ap_PDU s1apPDU = {0};
        ...
        log_msg(LOG_DEBUG,"Inside ics_req_attach_req processing");

	get_icsreq_protoie_value(&s1apPDU.value, g_icsreq);
	...
	if(g_icsreq->exg_max_dl_bitrate <= UINT32_MAX &&
		g_icsreq->exg_max_ul_bitrate <= UINT32_MAX)
	{
        	req.ueag_max_dl_bitrate = g_icsreq->exg_max_dl_bitrate;
        	req.ueag_max_ul_bitrate = g_icsreq->exg_max_ul_bitrate;
	}
	else
	{
		/*TODO */
	...
	/* id-E-RABToBeSetupListCtxtSUReq */
	erab_setup_item *erab = &(s1apPDU.value.data[3].val.erab_to_be_setup_item_ctxt_su_req);
        req.erab_su_list.count = 1;
        for(int i = 0; i < req.erab_su_list.count; i++)
        {
		/* TODO : Remove hardcoded value of erab id */
                req.erab_su_list.erab_su_item[i].e_RAB_ID = 5;
                req.erab_su_list.erab_su_item[i].e_RAB_QoS_Params.qci =
                                erab->e_RAB_QoS_Params.qci;
                req.erab_su_list.erab_su_item[i].e_RAB_QoS_Params.arPrio.prioLevel =
                                (erab->e_RAB_QoS_Params.arPrio.prioLevel);
                req.erab_su_list.erab_su_item[i].e_RAB_QoS_Params.arPrio.preEmptionVulnebility = 0;
                req.erab_su_list.erab_su_item[i].e_RAB_QoS_Params.arPrio.preEmptionCapab = 0;
                req.erab_su_list.erab_su_item[i].gtp_teid =
                                htonl(erab->gtp_teid);
                req.erab_su_list.erab_su_item[i].transportLayerAddress =
				htonl(erab->transportLayerAddress);
		if (g_icsreq->nasMsgSize != 0) 
		{
			req.nasMsgLen = g_icsreq->nasMsgSize;
			memcpy(req.nasMsgBuf, g_icsreq->nasMsgBuf, req.nasMsgLen);
			memcpy(&req.erab_su_list.erab_su_item[i].nas, &g_icsreq->nasPDU,
					sizeof(g_icsreq->nasPDU));
		}
        }
        memcpy(&(req.sec_key), &(g_icsreq->sec_key), KENB_SIZE);

	if (g_icsreq->ho_restrict_list_presence)
	{
		memcpy(&req.ho_restrict_list, &g_icsreq->ho_restrict_list, sizeof(g_icsreq->ho_restrict_list));
	}
        ...
        if(ret == -1)
        {
                log_msg(LOG_ERROR, "Encoding ics_req_attach failed.");
		                return E_FAIL;
        }
...
icsreq_handler(void *data)
{
	...
	ics_req_attach_processing((struct init_ctx_req_Q_msg *)data);
