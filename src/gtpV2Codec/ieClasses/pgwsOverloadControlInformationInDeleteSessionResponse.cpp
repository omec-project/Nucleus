                          ...
                          const &data)
{
    ...
    if (data.listOfAccessPointNameIePresent)
    {
        ...
        GtpV2Ie::encodeGtpV2IeHeader(buffer, header);
        startIndex = buffer.getCurrentIndex(); 
        ApnIe listOfAccessPointName=
        dynamic_cast<
        ApnIe&>(GtpV2IeFactory::getInstance().getIeObject(ApnIeType));
        rc = listOfAccessPointName.encodeApnIe(buffer, data.listOfAccessPointName);
        endIndex = buffer.getCurrentIndex();
        length = endIndex - startIndex;
...
(PgwsOverloadControlInformationInDeleteSessionResponseData const &data, Debug &stream)
{
    ...
    if (data.listOfAccessPointNameIePresent)
    {
        ...
        dynamic_cast<
        ApnIe&>(GtpV2IeFactory::getInstance().getIeObject(ApnIeType));
        listOfAccessPointName.displayApnIe_v(data.listOfAccessPointName, stream);
